...
# Importing the ttk module from the tkinter package.
from tkinter import ttk

# Importing the filedialog module from the tkinter package.
from tkinter import filedialog

# Importing the messagebox module from the tkinter package.
from tkinter import messagebox

# The above code is importing the pickle module.
import pickle
...
# Creating a threading.Lock object.
import threading

# The above code is using the struct module to unpack the data from the file.
import struct


# Setting the DPI awareness of the process to Per-Monitor DPI awareness.
try:
...
print("\n''It's a Client Server Enter the HOST to Start chating''")


# HOST = "127.0.0.1"
HOST = input("Enter host to connect : ")
PORT = 9999
# PORT = int(input("Entr Port : "))

class FirstScreen(tk.Tk):
    # It creates a GUI window with a username entry box and a submit button.
    # When the submit button is clicked, it sends the username to the server and receives a list of all
    # the connected clients.
    # It then creates a new window with a list of all the connected clients and a text entry box.
    def __init__(self):
        """
        It creates a GUI window with a background image, a label, an entry box, a button and a profile
        image
        """
        super().__init__()
        ...
        self.username_entry.bind('<Return>',lambda x:  self.process_data())

        self.mainloop()

    def add_photo(self):
        """
        It opens a file dialog, gets the file path, gets the file name, gets the file extension, resizes
        the image, saves the resized image, closes the image, opens the resized image, converts the
        image to a PhotoImage, and then displays the image in a label
        """
    ...
    def process_data(self):
        """
        It connects to the server, sends the user's name and image, receives the list of connected users
        and their images, and then opens a new window for the chat
        :return: The return value is a tuple of the form (sock, address) where sock is a socket object
        usable to send and receive data on the connection, and address is the address bound to the
        ...
        if self.username_entry.get():
            ...
            print(clients_data_size_bytes)
            clients_data_size_int = struct.unpack('i', clients_data_size_bytes)[0]
            b = b''
            while True:
               # The  code is receiving the data from the client.
            ...
            ChatScreen(self, self.first_frame, client_socket, clients_connected, user_id, self.user)


class ChatScreen(tk.Canvas):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id, user):
        """
        It's a function that creates a GUI for a chat application
        
        :param parent: The parent widget
        :param first_frame: The first frame that is displayed when the program is run
        :param client_socket: The socket object that will be used to send and receive data
        :param clients_connected: A dictionary that contains the user_id as the key and a tuple of the
        user's name, image bytes and image extension as the value
    ...
    def receive_data(self):
        """
        It receives data from the server and if the data is a notification, it receives the data size
    ...
    def on_closing(self):
        """
        It asks the user if they want to disconnect from the server, and if they do, it removes the
        user's image from the server and closes the socket
        """
        if self.window == 'PyChat':
    ...
    def received_message_format(self, data):
        """
        It takes a dictionary of data, unpacks it, and then creates a frame with a label and an image,
        and then packs it into the canvas
        
        :param data: The data that is received from the server
        """
        ...
        t_label = tk.Label(m_frame, bg="#595656", fg="white", text=user,font="lucida 7 bold", justify="left", anchor="w")
        t_label.grid(row=0, column=1, padx=2, sticky="w")

        t_label2 = tk.Label(m_frame, bg="#595656",fg="white", text=datetime.now().strftime('%H:%M'), font="lucida 7 bold",
                           justify="left", anchor="w")
        t_label2.grid(row=0, column=0, padx=2, sticky="w")
        ...
        i_label.image = im
        i_label.grid(row=1, column=0, rowspan=2, sticky="w")
    ...
    def sent_message_format(self, event=None):
        """
        It takes the message from the entry box, formats it, and sends it to the server
        ...
        if message.strip():
            ...
            data = {'from': from_, 'message': message, "user": self.user}
            ...
            t_label = tk.Label(m_frame, bg="#595656", fg="white", text=self.user,font="lucida 7 bold", justify="right", anchor="e")
            t_label.grid(row=0, column=0, padx=2, sticky="e")
            
            t_label2 = tk.Label(m_frame, bg="#595656", fg="white", text=datetime.now().strftime('%H:%M'),
                               font="lucida 7 bold", justify="right", anchor="e")
            t_label2.grid(row=0, column=2, padx=2, sticky="e")
    ...
    def notification_format(self, data):
        """
        It creates a frame, adds a label with the time, adds a label with the message, and then packs
        the frame.
        
        :param data: The data that is sent from the server
        """
    ...
    def clients_online(self, new_added):
        """
        It takes a dictionary of user_id: (name, image_bytes, extension) and creates a label for each
        user_id in the dictionary.
        
        :param new_added: This is a list of the new user that has been added to the chat
        """
        if not new_added:
            pass
    ...
    def remove_labels(self, client_id):
        """
        It removes a label from a dictionary of labels
        
        :param client_id: The id of the client that is being removed
        """
    ...
    def insert_emoji(self, x):
        """
        It inserts the text of the button that was clicked into the entry widget.
        
        :param x: The event that triggered the function
        """
        self.entry.insert("end-1c", x.widget['text'])

    def first_screen(self):
        """
        It destroys the current frame, changes the geometry of the parent window, and then packs the
        first frame
        """
        self.destroy()
...
# Calling the function FirstScreen()
FirstScreen()